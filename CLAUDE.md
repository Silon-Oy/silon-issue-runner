...
tärkeysjärjestyksessä** (`LABEL_ORDER`): `needs-human` → "Vaatii ihmisen", `auto-clean` →
"Siivousjonossa", `auto-clean-skipped` → "Siivous epäonnistui". Chipin **teksti tulee JS:n omasta
staattisesta kartasta** (`LABEL_CHIPS`), ei koskaan raa'asta GitHub-merkkijonosta — taulukkoa vain
testataan jäsenyydeltä (`hasLabel`), joten skeeman kasvattama labeli ei voi vuotaa. Labelit ovat
issue-tasoisia ⇒ chip näkyy **kaikilla** saman issuen riveillä (speksin reunaehto). **"Siivoa"-nappi
disabloituu** (ei hiljaista no-op-klikkausta) ja nimeää syyn ("Jo siivousjonossa (auto-clean-label
issuella)") kun issuella on jo `auto-clean` — data-vetoisesti (`github.issue_labels` `status.json`ista),
joten tila **säilyy sivun päivityksen yli** ilman selainmuistia (AC3+AC4). Sivu on yhä dataton runko.
Vartijat: `tests/test-status-github.sh` (labelien keräys + valkolista + cache + fail-soft),
`test-status-render.sh` (chip-sanamuodot + `issue_labels`-luku + Siivoa-disablointi + XSS-escape),
`test-status-schema.sh` (`issue_labels` ei vuoda ajon päätasolle).

**#125 teki suljettujen issueiden detail-luvuista pysyväisluonteisia** ilman skeema- tai
sivupuolen muutosta — korjaus on **puhtaasti `status.sh --github`-cachen sisäinen**. Suljettu
issue on terminaalitila (tila, sulkusyy, otsikko, labelit eivät muutu), joten `status_github_issue_detail`in
tulos **kannetaan cache-missin yli** sen sijaan että se luettaisiin uudelleen joka kerta kun
`RUN_ISSUES_STATUS_CACHE_TTL` umpeutuu. Ennen tätä cache-miss alusti `issue_details`in `{}`:ksi ja
luki koko joukon uudelleen (mitattu Studiolla 318 turhaa `gh issue view` -kutsua per päivitys, joka
osaltaan ylitti GitHubin kiintiön 2026-08-28/29); vakaassa tilassa **318 → ~0 kutsua**. Cache-miss
alustaa `issue_details`in nyt **vanhentuneen** cache-merkinnän detail-tiedoista, ja olemassa oleva
"jo ratkaistu ⇒ ohita luku" -tarkistus hoitaa loput. Kolme vartiota pitää sen rehellisenä: (1) **oma
vanhenemisikkuna** — jokaiseen detail-merkintään stampataan `fetched_epoch` luentahetkellä, ja
carry-forward kantaa vain ne, joiden ikä on alle `RUN_ISSUES_STATUS_DETAIL_TTL`in (oletus 86400 s);
`0` poistaa carry-forwardin, ja aikaleimaton legacy-merkintä käsitellään vanhentuneena (kertaluontoinen
täysi luku deployn jälkeen); (2) **uudelleenavattu issue** — carry-forwardista pudotetaan jokainen
avain, joka esiintyy tuoreessa avoimien issueiden kartassa, koska `issuemeta`-merge (`(openmap) + $det`)
antaa etusijan oikealle ⇒ kannettu `CLOSED` muutoin varjostaisi tuoreen `OPEN`in ja uudelleenavattu
issue näkyisi ikuisesti suljettuna; (3) **#96:n legacy-vartija säilyy** — tuore merkintä ilman
`issue_details`-avainta pakottaa yhä täyden refetchin. `fetched_epoch` on cache-sisäinen: `issuemeta`in
kuluttaja valkolistaa `title`/`state`/`state_reason`/`labels`, joten se ei koskaan vuoda emittoituun
`github`-objektiin. Epäonnistunutta lukua ei negatiivisesti cachetata (nykyinen käytös), joten se
yritetään uudelleen. Vartijat `tests/test-status-github.sh` (nollakutsu cache-missillä, uusi suljettu
⇒ täsmälleen yksi luku, uudelleenavattu ⇒ `OPEN`, detail-TTL:n umpeutuminen, legacy-vartija).
...
| `machine-env.sh` | Koneistokohtaisen env-tiedoston (`RUN_ISSUES_ENV_FILE`, oletus `$HOME/.config/run-issues/env`) sourceaus **kutsujan etuoikeudella** (#144). `source_machine_env` — jaettu `orchestrate.sh`:n ja `pr-watch.sh`:n kesken, jotta sääntö on olemassa yhdessä paikassa. **Sääntö on nimiavaruus, ei poikkeuslista:** `RUN_ISSUES_*` ja `PR_WATCH_*` -muuttujissa kutsujan jo asettama arvo **voittaa** tiedoston (asetettu tyhjäksi = asetettu); kaikki muu (salaisuudet, joita kukaan ei aseta käsin) säilyttää `tiedosto voittaa` -semantiikan, joka on koko kanavan tarkoitus. Toteutus: `_machine_env_snapshot` (`printf %q` nimiavaruuden muuttujista) ennen sourcea, `eval` sen jälkeen. Loki `_machine_env_log`illa (`declare -F log` → stderr, sama kuvio kuin `run-terminate.sh`illa) — juuri se ero, jolla `pr-watch.sh`:n oma kopio aiemmin perusteltiin. Puhtaita funktioita. Vartija `tests/test-machine-env.sh` |
| `pr-watch-lib.sh` | PR:n luokittelu- ja merge-päätöslogiikka (irrotettu testattavaksi). Ml. `pr_last_decision` (#65): lukee state.jsonlin **hännästä** viimeisimmän `pr_classified`-päätöksen, jotta `pr-watch.sh` osaa vaieta toistuvan `SKIP_CLOSED`-tapahtumatrion. **#131:** `pr_decide` erottaa haun epäonnistumisen PR:n sulkeutumisesta uudella `SKIP_UNKNOWN`-paluuarvolla: tyhjä tai osittainen payload (tyhjä/puuttuva `.state` tai puuttuva `.labels`/`.statusCheckRollup`) tarkoittaa että `gh pr view` epäonnistui (rate limit, verkko, oikeudet), ei että PR on kiinni. Ennen tätä tyhjä `$json` tuotti saman `SKIP_CLOSED`in kuin suljettu PR, jolloin avoin `auto-merge`-PR saattoi jäädä mergeämättä koko rate-limit-episodin ajaksi **ilman yhtään lokiriviä** (SKIP_CLOSED on juuri se päätös jonka #65 vaimentaa toistuessaan). Fail-closed: SKIP_UNKNOWN ei koskaan mergeä/sulje/siivoa/labeloi. Täydellisyystarkistus lukee `.state`n **arvon** (ei-tyhjä merkkijono) mutta muiden **avaimen olemassaolon** — avoimen PR:n `labels:[]`/`statusCheckRollup:[]` (läsnä mutta tyhjä) on täysi payload, ei osittainen. `watch_one` käsittelee `SKIP_UNKNOWN`in aina-lokitettuna ohituksena (return 4) jota **ei kirjata `state.jsonl`iin `pr_classified`-päätöksenä** (emit_events=0), jottei tuntematon tila saastuta `pr_last_decision`in hännästä luettavaa historiaa (ja mahdollisen #130-suodatuksen kautta pudottaisi PR:ää pysyvästi). Vartija `tests/test-pr-watch-decision.sh` (pure erottelu + Layer 2 -integraatio: tyhjä payload ⇒ ei `pr_classified`, ei mergeä; suljettu PR ⇒ kirjautuu, kontrasti) |
...
- **`status.sh --github`** (#60) — kokonaistilan opt-in GitHub-rikastus. Ilman lippua `status.sh`
  ...
  `issue_state_reason`in (ml. PR-rivit, näkymää varten) ja suljetun issuen otsikon; #106 lisäsi
  `github.issue_labels`in (valkolistattu kolmikko `auto-clean`/`auto-clean-skipped`/`needs-human`,
  samasta issue-meta-kartasta, ei koskaan koko labelijoukko); luokittelu
...
- **`lib/github-app-auth.sh`** — GitHub App -identiteetti henkilökohtaisen tokenin sijaan.
  ...
  Kattaa **kirjoitusten lisäksi raskaimmat luvut** (#127): `pick_oldest_candidate`,
  `issue_assignees`, `epic_list_open` ja `scan_clean`in labelikysely reititetään `_issue_gh`:n
  kautta (`gha_with_token`), joten poiminta- ja skannausvolyymi maksetaan **Appin omasta
  15k/h-kiintiöstä**, ei henkilökohtaisesta tilistä. Identiteetti ei muuta lukujen sisältöä, vain
  sen **kenen kiintiöstä** ne kuluvat — juurisyy sille, että automaatio ja käsityö täyttivät
  ennen saman kiintiön ja pysäyttivät toisensa. **`poller.sh` sourcaa nyt tämän moduulin** (ennen
  vain `orchestrate.sh`/`pr-watch.sh`/`status.sh`), jotta pollerin oma poiminta osaa reitittää
  Appin kautta; ilman App-konfiguraatiota se on hyvänlaatuinen no-op (paljas `gh`). Pollerin
  App-identiteettikonfiguraatio (ID:t + `.pem`-polku, **ei avainmateriaali**) tulee `poller.env`istä
  — poller ei koskaan sourcea `env`iä (§7). Poissulut ennallaan: claimin `@me`-assignaatio ja
  `verify_claim`in `gh api user` pysyvät henkilökohtaisella tilillä (App ei voi olla assignee),
  ei-origin-remote ohittaa Appin aina (org-kohtainen asennus), ja per-issue-luvut
  (`count_open_blockers`/`list_blocked_by`/`is_epic`) jäävät paljaaseen `gh`:hon (matalavolyymiset,
  #127 scope-out: ei kutsumäärien vähentämistä). Token-cache kestää volyymin: N peräkkäistä
  `_issue_gh`-lukua samassa prosessissa tekee **yhden** token-mintin (cache-osuma), vartija
  `tests/test-github-app-auth.sh` TEST 8–9.
...
- **Koneen env-tiedosto ohitti testien stubin — korjattu (#144).** `orchestrate.sh` ja
  `pr-watch.sh` sourceavat `$HOME/.config/run-issues/env`in prosessin sisällä. Tiedosto on
  ...
  stubbaavat agentin `RUN_ISSUES_CLAUDE_CMD="$BIN/claude"`illa, joten koneella jonka env-tiedosto
  exporttaa oikean CLI:n `tests/run-all.sh` **käynnisti oikeita, laskutettavia 3600 s
  agenttiajoja** väliaikaisrepoa vasten — mitattu 2026-08-31, eikä mikään tuloste kertonut siitä.
  Sama testi teki kahdella koneella kaksi eri asiaa. **Korjaus** on jaettu `lib/machine-env.sh`
  (§6): nimiavaruussääntö `RUN_ISSUES_*`/`PR_WATCH_*` ⇒ kutsuja voittaa, muut ⇒ tiedosto voittaa.
  **Poikkeuslista olisi ollut väärä muoto** — se jättäisi seuraavan lisätyn muuttujan kattamatta,
  mikä on täsmälleen se tapa jolla tämä vika säilyi (`poller.env`in tarkoituksellinen
  "tiedosto voittaa" nimeää poikkeuksensa rakenteellisiksi §7:ssä, eikä samaa päättelyä koskaan
  laajennettu tänne). Sivuseuraus jonka tämä poisti: pollerin elävät agenttiprosessit ja testin
...
- **`state.jsonl`-kohina PR-vahdista — kasvun lähde tukittu (#65), takautuva siivous jäljellä.**
  ...
  **#131:n täydennys tähän sääntöön:** haun epäonnistuminen (`SKIP_UNKNOWN`, §6
  `lib/pr-watch-lib.sh`) **ei koskaan päädy `state.jsonl`in historiaan** — `watch_one` käsittelee
  sen aina-lokitettuna ohituksena mutta jättää `pr_classified`-rivin kirjaamatta (emit_events=0).
  Muuten tyhjän/osittaisen payloadin tuottama tuntematon tila opettaisi `pr_last_decision`in
  hännästä luettavaan historiaan väärän päätöksen, ja rate-limit-episodin aikainen kirjaus jäisi
  pysyvästi kummittelemaan (ja #130:n skannaussuodattimen kautta pudottaisi PR:n ajosta
