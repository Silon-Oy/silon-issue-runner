...
description: Use when working in a repository watched by the claude-issue-runner / run-issues automation — you see an auto-run, needs-human, waiting, wip, epic, auto-clean or auto-merge label; a branch whose name starts with auto-run/ (e.g. auto-run/<repo>-issue-<N>-<slug>); a run.json artifact or a .claude/run-issues-archive/ directory; an issue comment carrying a <!-- run-issues:… --> marker; a bot-opened PR closing an issue; or the /run-issues, /run-epic, /pr-watch or /cleanup-run commands. Covers what the system is, when it picks an issue up, what every label means and who writes it, how to read and unstick a blocked or stalled run, and which command or script to reach for.
...
2. Poller poimii vanhimman ehdot täyttävän issuen ja **assignoi sen itselleen** varaukseksi.
...
|---|---|---|---|
| `auto-run` | sinä (tai epic-propagointi) | sinä | Poimintaehto. Nimi tulee konfiguraatiosta (`RUN_ISSUES_LABELS_CSV`), ei koodista |
...
- **`needs-human` ei estä poimintaa issuella** — assignaatio estää. PR:llä se pidättää vahdin,
  mutta vain kun vahti itse lisäsi sen CI-korjauksen luovutuksessa; käsin lisättynä muulle
  PR:lle se ei jarruta mergeä.
...
varausmekanismina. Kaksi seurausta:

- **Käsin assignattu issue ei koskaan lähde automaatioon.** Jos haluat tehdä issuen itse,
  assignoi se itsellesi — se on `wip`-labelia vahvempi keino.
- **Epäonnistunut ajo jättää issuen varatuksi itselleen**, ja issue palaa automaatioon vasta
  siivouksen jälkeen: poller ei poimi samaa issueta yhä uudelleen samaan seinään.
...
1. **Assignaatio.** Onko issuella assignee? **Kuka tahansa assignee estää poiminnan** — myös
   sinä itse, myös aiemman epäonnistuneen ajon jättämä varaus. Tämä on ylivoimaisesti yleisin
   syy. Poista assignaatio tai siivoa vanha ajo.
...
3. **Estolabelit.** Onko issuella `waiting`, `wip`, `epic` tai `auto-clean`? Kaikki neljä
   estävät poiminnan. Huom: `needs-human` **ei** estä poimintaa — assignaatio estää.
...
- `/run-issues` — aja orkestraattori nimetylle issuelle tai vanhimmalle omistamattomalle.
