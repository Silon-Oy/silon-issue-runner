...
#
# Usage:
#   auto-clean.sh --repo <repo-root> --issue <N> [--dry-run]
#
# Exit codes:
...
add_skipped_label() {
  ...
  (
    cd "$REPO_ROOT"
    gh label create "$SKIPPED_LABEL" \
      --color "ededed" \
      --description "auto-clean skipped this issue; needs human attention" \
      >/dev/null 2>&1 || true
    gh issue edit "$ISSUE_NUM" --add-label "$SKIPPED_LABEL" >/dev/null 2>&1 || true
  )
}

# ---------- 1. lock ----------
# Acquire the per-issue lock BEFORE any teardown. If a run (orchestrate.sh,
# restart, continue) currently holds it, bail with rc=3 — the poller will retry
# on a later tick once the run releases the lock.
if ! lock_issue "$ISSUE_NUM"; then
  log "lock held for issue #$ISSUE_NUM — a run is in progress; will retry later"
  exit 3
fi

# IMPORTANT: cleanup-run.sh removes the per-issue lock as part of its teardown
# (it sources lib/locking.sh and rm -rf's the issue-N.lock dir). So once we hand
...
# Count run-dirs whose run.json .issue_number matches, and how many of those
# are status=completed. bash 3.2 compatible — plain counters, no assoc arrays.
total=0
completed=0
shopt -s nullglob
...
if [ "$total" -eq 0 ]; then
  ...
  # issue auto-clean-skipped so we stop re-emitting it. exit 5 is the defensive
  # "no local candidate" signal; the poller already gates network calls to
  # local candidates, so reaching here usually means a genuine cross-machine run.
  log "no local run-dirs for issue #$ISSUE_NUM (cross-machine?)"
  if [ "$DRY_RUN" = "1" ]; then
...
Tällä koneella (\`$(hostname -s)\`) ei löytynyt issueen #$ISSUE_NUM liittyviä run-direjä. Resurssit ovat todennäköisesti toisella koneella.
...
if [ "$completed" -gt 0 ]; then
  ...
  # runs) could orphan an open PR. The whole issue is handed to a human; the
  # mixed case (some completed, some not) is treated identically to all-completed.
...
log "tearing down issue #$ISSUE_NUM ($total non-completed run-dir(s))"
CLEANUP_ARGS=(--repo "$REPO_ROOT" --issue "$ISSUE_NUM" --yes)
...
Issueen #$ISSUE_NUM liittyvät paikalliset väliaikaisresurssit on siivottu koneella \`$(hostname -s)\`:
...
(
  cd "$REPO_ROOT"
  gh issue edit "$ISSUE_NUM" --remove-label "$RUN_ISSUES_CLEAN_LABEL" >/dev/null 2>&1 || true
)

log "done: issue #$ISSUE_NUM cleaned and closed"
exit 0
