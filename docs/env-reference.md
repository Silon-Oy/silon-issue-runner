...
| `RUN_ISSUES_LOCK_ROOT` | macOS: `$HOME/Library/Application Support/run-issues/locks`; muut: `${XDG_STATE_HOME:-$HOME/.local/state}/run-issues/locks` | Lukkohakemistojen juuri. Oletus haarautuu `uname -s`:llä (`lib/paths.sh`: `default_lock_root`); haara on `Darwin` ⇒ macOS, **kaikki muu** ⇒ XDG, jotta myös `MINGW64_NT-*` osuu oikein |
...
| `RUN_ISSUES_POLLER_HOSTS` | *(ei oletusta — pakollinen)* | Pilkuin/välilyönnein eroteltuja glob-kuvioita, verrataan koneen lyhyeen konenimeen (`runner_host`, `lib/host.sh`). `*` sallii kaikki. Ei osumaa ⇒ poller exittaa 0 luomatta mitään. **Asettamatta poller ei aja millään koneella** ja kirjoittaa yhden rivin — sekä stderriin että omaan lokiinsa, koska LaunchAgent-ajossa stderr ei mene mihinkään — joka nimeää muuttujan ja `poller.env`-polun (#152) |
| `RUN_ISSUES_WATCHLIST` | *(tyhjä)* | Watchlistin polku. Asetettuna se on **ainoa** ehdokas — osumaton override on virhe, ei fallback |
| `RUN_ISSUES_LOG_DIR` | macOS: `$HOME/Library/Logs`; muut: `${XDG_STATE_HOME:-$HOME/.local/state}/run-issues/logs` | Kaikkien neljän lokitiedoston hakemisto per poller (`.log`, `.runs.log`, `.stdout.log`, `.stderr.log`). Oletus haarautuu `uname -s`:llä (`lib/paths.sh`: `default_log_dir`). Myös jaettu rate-limit-tilatiedosto ja PR-vahdin rotaatiokursori asuvat täällä, joten oletuksen vaihtuessa ne aloittavat puhtaalta pöydältä — molemmat ovat fail-soft |
...
| `RUN_ISSUES_LOG_DIR` | macOS: `$HOME/Library/Logs`; muut: `${XDG_STATE_HOME:-$HOME/.local/state}/run-issues/logs` | Skripti ohjaa oman stdout/stderrinsä `status-render.stdout.log`/`.stderr.log`-tiedostoihin täältä, kun ei aja TTY:llä (plistissä ei loki-avaimia, §11) |
...
Nimeäminen on se osa joka maksaa itsensä takaisin: luku `2` ei erota kaksiytimistä konetta
väärin vastanneesta mittarista, ja vain lähde kertoo kummasta on kyse. Poikkeus, jota varten
override on olemassa: CPU-kiintiöidyssä kontissa suurin vastaus voi ylittää kiintiön, koska
