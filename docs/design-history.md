# Suunnitteluhistoria — miksi asiat ovat niin kuin ovat

**Mikä tämä on:** tilannekuva `CLAUDE.md`:n teknisestä kerronnasta ennen sen tiivistämistä
(lähde: commit `fef3e96`, 2026-09-01). Se sisältää issue-kohtaiset perustelut ja
muutoshistorian luvuille "tilakone", "exit-koodit", "lib/", "opt-in-mekanismit",
"LaunchAgent" ja "avoimet asiat".

**Mikä tämä ei ole:** ei lähde eikä ylläpidetty. Elävät lähteet ovat:

| Tieto | Lähde |
|---|---|
| Exit-koodit | Skriptin otsikkokommentti (`# Exit codes:`); `README.md` §9 on `tests/test-readme.sh`:n vartioima peilaus |
| Ympäristömuuttujat | Koodin `${VAR:-oletus}` + skriptin `# Env:`; `docs/env-reference.md` on peilaus |
| Invariantit ja mitatut rajoitteet | `CLAUDE.md` |
| Miksi yksittäinen muutos tehtiin | Kyseisen issuen PR-kuvaus ja `git log` |

**Älä lisää tähän uutta.** Uuden muutoksen perustelu kuuluu sen omaan PR-kuvaukseen. Tämä
tiedosto on olemassa, jotta kuukauden aikana kertynyt perusteluaineisto ei katoa
tiivistyksessä — ei jatkokasvualustaksi.
...
- **`status-digest.sh` + `gws`** (#61) — työntökooste huomiota vaativista ajoista.
  ...
  kun erään kohderepon issue #92 odotti 71 vrk `awaiting_clarification`-tilassa ilman
  että mikään työnsi tietoa. Nojaa **vain** paikalliseen JSONiin (`github: null` on
  laillinen), ei gh-rikastukseen.
